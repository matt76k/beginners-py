...
逐次については[データ](data.md)の章で何度も見てきました。
...
このプログラムが書ければ、逐次は理解しているといえるでしょう。
...
この章では、逐次以外の**分岐**、**反復**について学習します。
さらに、3つの制御を使った問題と**変数スコープ**について説明します。
...
これは、「偶数です」と表示されます。
なぜなら、条件式は逐次に従って実行されるため、最初の`n % 2 == 0`の条件が`True`になり、`elif`の条件は実行されません。
if文を書くときは、逐次に従って条件式が実行されることを意識しましょう。

複数の条件を組み合わせるには、**論理演算子**を使います。
`and`は両方の条件が`True`のときだけ`True`、`or`はどちらか一方が`True`なら`True`になります。
...
繰り返しの時に使われる変数のことを特別に**ループ変数**といいます。
...
インデックスと値を同時に取り出すには、`enumerate`を使います。
...
`enumerate`は`(インデックス, 値)`の組を順番に返します。
...
プログラムを書く上では、変数がどこで使えるかという**変数スコープ**についての理解が重要になります。

変数スコープとは、変数が有効である範囲のことです。
Pythonでは、変数がどこで定義されたかによって、その変数がアクセスできる範囲が決まります。

多くのプログラミング言語では、
forループの中で定義された変数はループ内でしか使えません。
しかし、Pythonではforループを抜けた後も使えます。
...
また、Pythonではforループの中で定義した変数もforループを抜けた後も使えます。
...
print(f"最後のnum: {num}")      # 30
print(f"最後のresult: {result}") # 60
```

以上のことから、forループを使う時は、注意が必要です。
特に、forループで既存の変数と同じ名前を使うと、元の値が上書きされてしまいます。
...
これを避けるため、異なる変数名を使うか、元の値を別の変数に保存する必要があります。

```python
original_x = "元の値"
print(f"ループ前: {original_x}")

for item in ["A", "B", "C"]:
    print(f"ループ中: {item}")
    
print(f"ループ後: {original_x}")
```

for文の中にfor文（二重ループ）を書くときも、ループ変数の名前が被らないように気をつけましょう。

例えば、xy平面上で、xが0から1、yが0から2までの格子点を出力するコードは以下のように書けます。
...
これは意図した通りの結果です。
xが0のときにyが0, 1, 2と変化し、次にxが1のときにyが再び0, 1, 2と変化しています。

しかし、以下のように同じ変数名iを使ってしまうとどうなるでしょうか。
...
これは意図した結果ではありません。
内側のループでiの値が上書きされてしまい、x座標とy座標が常に同じ値になってしまいます。
また、外側のループの制御も正しく動作しなくなります。

このように、二重ループでは必ず異なる変数名を使うと良いでしょう。
変数名が被ると、プログラムが予期しない動作をしてしまい、バグの原因となります。